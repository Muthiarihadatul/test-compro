...
export default function PublicationList() {
      ...
      try {
        ...
        setPublications(
          (json ?? []).map((t: any) => ({
            id: t.id,
            title: t.title,
            authors: Array.isArray(t.authors)
              ? t.authors.join(', ')
              : t.authors,
  ...
  ======================= */
  const allSelected =
    selected.length === publications.length && publications.length > 0;
  ...
  /* =======================
     UI STATES
  ======================= */
  if (!query) return <p>Masukkan kata kunci pencarian</p>;
  if (loading) return <p>Loading publications...</p>;
  if (!publications.length)
    return <p>No publications found for "{query}"</p>;
        ...
        <div className="flex gap-2">
          ...
          <div className="relative" ref={sortRef}>
            <button
              ...
              className="flex items-center gap-2 bg-red-700 text-white px-4 py-1.5 rounded-full text-sm font-medium hover:bg-red-800"
            >
              {sortBy === 'title'
                ? 'Title (A–Z)'
                : sortBy === 'cited'
            ...
            <div
              className={`absolute right-0 mt-2 w-44 bg-red-50 rounded-2xl shadow z-30 transition-all ${
                openSort
            ...
            >
              <button
                ...
                className="w-full text-left px-4 py-3 text-sm text-red-800 hover:bg-red-700 hover:text-white"
              >
                Title (A–Z)
              </button>
              <button
